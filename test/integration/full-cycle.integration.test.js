...
describe("Integration - Full 5 Stage Lifecycle", function () {
  ...
  it("should execute all stages without error", async () => {
    ...
    gw = await connectAs("LifecycleUser");
    net = await gw.getNetwork("rebar-channel");
    let lifecycleContract = net.getContract("rebarcc", "LifecycleContract");
    await lifecycleContract.submitTransaction(
      "recordTransport",
      JSON.stringify({ packageId: "PKG_FULL_1", destination: "CustZ" })
    );
    await gw.disconnect();

    // Stage 5: Sales Order + Payment
    gw = await connectAs("FinanceUser");
    net = await gw.getNetwork("rebar-channel");
    let financeContract = net.getContract("rebarcc", "FinanceContract");
    await financeContract.submitTransaction(
      "createSalesOrder",
      JSON.stringify({
        orderNo: "SO_FULL_001",
        customer: "CustZ",
    ...
    const queryRes = await lifecycleContract.evaluateTransaction(
      "getProductLifecycle",
      "S_FULL_001"
    );
